...
) as Match.MatcherTypeId

const TypeMatcherProto: Omit<Match.TypeMatcher<any, any, any, any>, "cases"> = {
  [TypeId]: {
    _input: identity,
    _filters: identity,
    _remaining: identity,
  ...
  _tag: "TypeMatcher",
  add<I, R, RA, A>(
    this: Match.TypeMatcher<any, any, any, any>,
    _case: Match.Case,
  ): Match.TypeMatcher<I, R, RA, A> {
...
function makeTypeMatcher<I, R, RA, A>(
  cases: ReadonlyArray<Match.Case>,
): Match.TypeMatcher<I, R, RA, A> {
...
const ValueMatcherProto: Omit<
  Match.ValueMatcher<any, any, any, any, any>,
  "provided" | "value"
> = {
  [TypeId]: {
    _input: identity,
  ...
  _tag: "ValueMatcher",
  add<I, R, RA, A, Pr>(
    this: Match.ValueMatcher<any, any, any, any, any>,
    _case: Match.Case,
  ): Match.ValueMatcher<I, R, RA, A, Pr> {
...
/** @internal */
export const type = <I>(): Match.Matcher<
  I,
  Types.Without<never>,
  I,
  never,
...
export const value = <const I>(
  i: I,
): Match.Matcher<I, Types.Without<never>, I, never, I> =>
  makeValueMatcher(i, Either.left(i))
...
>(
  ...
  const predicates: Array<Match.When> = []
...
<I, R, A, Pr>(
  self: Match.Matcher<I, R, RA, A, Pr>,
...
export const either: <I, F, R, A, Pr>(
  ...
  : Either.Either<R, Unify<A>> =
    (<I, R, RA, A>(self: Match.Matcher<I, R, RA, A, I>) => {
...
export const option: <I, F, R, A, Pr>(
  ...
  : Option.Option<Unify<A>> =
    (<I, A>(self: Match.Matcher<I, any, any, A, I>) => {
...
export const exhaustive: <I, F, A, Pr>(
  self: Match.Matcher<I, F, never, A, Pr>,
) => [Pr] extends [never] ? (u: I) => Unify<A> : Unify<A> = (<I, F, A>(
  self: Match.Matcher<I, F, never, A, I>,
) => {
